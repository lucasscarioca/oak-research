  ...
  const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.pathname === "/provider/config" && method === "PUT") {
      ...
      state.providerConfig = {
        ...
        validation_message:
          apiKey === "valid-key" ? null : "API key not valid. Please pass a valid API key.",
    ...
    if (url.pathname === "/diagnostics" && method === "GET") {
      return jsonResponse({
        ...
        provider_test_result: {
          ...
          message:
            state.providerConfig.validation_status === "valid"
    ...
    if (url.pathname === "/sources" && method === "POST") {
      ...
      const responseRecord = {
        ...record,
        metadata: { ...record.metadata, raw_bytes_length: rawBytes.length },
      };
    ...
    if (url.pathname.startsWith("/runs/") && url.pathname.endsWith("/stream") && method === "GET") {
      ...
      if (run) {
        ...
        run.blocked_reason =
          run.status === "blocked" ? "Insufficient grounding in notebook sources" : null;
        run.finished_at = "2026-04-24T00:00:00Z";
        run.answer = {
          id: run.id * 10,
...
describe("Grounded shell", () => {
  it("surfaces blocked attempts until Gemini is validated and allows provider save/test flow", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/Gemini provider configuration is not ready/i)).toBeInTheDocument(),
    );
    ...
    await waitFor(() =>
      expect(
        screen.queryByRole("dialog", { name: /Provider configuration/i }),
    ...
    await waitFor(() =>
      expect(screen.getByText(/Saved key tested successfully/i)).toBeInTheDocument(),
    );
    expect(fetchMock).toHaveBeenCalledWith(
  ...
  it("submits URL and file source payloads through the add-source modal", async () => {
    ...
    await user.type(
      screen.getByLabelText(/Manual fallback text/i),
    ...
    const textUpload = new File(["Body text for the article"], "article.txt", {
      type: "text/plain",
    });
    ...
    const postCalls = fetchMock.mock.calls.filter(
      ([input, init]) => String(input).includes("/sources") && (init?.method ?? "GET") === "POST",
    );
  ...
  it("surfaces inline source-title edit failures instead of throwing", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/Unable to update source title/i)).toBeInTheDocument(),
    );
  ...
  it("retries failed source ingestion from the sources tab", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText("Queued", { selector: ".badge.source-status" })).toBeInTheDocument(),
    );
  });
  ...
  it("streams question answers and opens cited source details", async () => {
    ...
    const runCalls = fetchMock.mock.calls.filter(
      ([input, init]) => String(input).includes("/runs") && (init?.method ?? "GET") === "POST",
    );
    ...
    await waitFor(() =>
      expect(screen.getAllByText(new RegExp(`Rerun of #${originalRunId}`)).length).toBeGreaterThan(
        0,
      ),
    );
  });
