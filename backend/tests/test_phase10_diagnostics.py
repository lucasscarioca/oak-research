...
class Phase10DiagnosticsTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_diagnostics_reports_provider_and_recent_failure_state(self) -> None:
        ...
        self.assertEqual(response.status_code, 200)
        with patch("grounded.ingestion.process_source_payload", side_effect=IngestionError("boom")):
        ...
        with patch("grounded.answering.retrieve_relevant_chunks", return_value=[]):
        ...
        response = await client.post(
            "/runs",
            json={"question": "What stack does Grounded use when the worker crashes?"},
        )
        self.assertEqual(response.status_code, 200)
        ...
        with (
            ...
            patch(
                "grounded.answering.stream_gemini_text",
        ...
        self.assertIn("Grounded overview", recent_job_labels)
        self.assertIn("Broken URL source", recent_job_labels)
        self.assertIn("What is the capital of Mars?", recent_job_labels)
        self.assertIn("What stack does Grounded use when the worker crashes?", recent_job_labels)
        ...
        self.assertIn("What stack does Grounded use when the worker crashes?", failure_labels)
